...
class MakeMoneyRecord extends StatefulWidget {
  @override
  _MakeMoneyRecordState createState() => _MakeMoneyRecordState();
}

class _MakeMoneyRecordState extends State<MakeMoneyRecord> {
  ...
  readData(){
    ...
    AppClass.readData(Api.mineBalanceRecordUrl).then((value){
      if(value!=null){
        setState(() {
          _list = value;
        });
  ...
  getDetailInfoData(){
    DioUtils.instance.post(Api.mineBalanceUrl, onFailure: (code,msg){
      ...
      if(response is Map){
        ...
        if(detailData['money_type']!=null){
          if(detailData['money_type'] is List){
            baozhangList = detailData['money_type'];
  ...
  getBaozhangData(Map data){
    ...
    DioUtils.instance.post(Api.mineBalanceRecordUrl,data: parameters, onFailure: (code,msg){
      reloadState();
    },onSucceed: (response){
      if(response is Map){
  ...
  showDataList(List dataList) {
    if (_page == 1) {
      _list.clear();
      AppClass.saveData(dataList, Api.mineBalanceRecordUrl);
    }
                              ...
                              children: <Widget>[
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: <Widget>[
                                      Expanded(child: AppButton(title: '余额类型',image: '账户明细-订单保障费',rightImage: 'ic_arrow_right',imageSize: 20,rightImageSize: 15,rightImageColor: AppColors.blackColor,buttonType: ButtonType.leftImageAndRightImage, onPress: (){
