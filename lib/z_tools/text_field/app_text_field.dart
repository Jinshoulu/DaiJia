...
class _AppTextFieldState extends State<AppTextField> {
  ...
  ///模式4
  createFourModel(bool isDark, ThemeData themeData){
    return Container(
      height: widget.height,
      padding: widget.edgeInsets??EdgeInsets.only(left: 16,right: 16),
      decoration: BoxDecoration(
  ...
  leftImage(){
    return  widget.leftImage==null?SizedBox():Container(
      width: 30,
      child: Center(child: LoadAssetImage(widget.leftImage,width: 15,fit: BoxFit.fitWidth,radius: 0.0,)),
    );
  }
  //中间输入框
  centerRadiusTextField(){
    ...
    return widget.allowChinese?Expanded(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            contentPadding: const EdgeInsets.symmetric(vertical: 12.0),
            hintText: widget.hintText,
            hintStyle: TextStyle(color: AppColors.black54Color,fontSize: Dimens.font_sp14),
            counterText: '',
            border: new OutlineInputBorder(
              borderSide: BorderSide.none,
            ),
  ...
  showEyes(){
    ...
    return !widget.showEyes ? Gaps.empty : Semantics(
      label: '密码可见开关',
      hint: '密码是否可见',
