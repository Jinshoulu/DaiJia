...
class DateFormats {
  ...
  static String mo_d_h_m_s = "MM-dd HH:mm:ss";
...
class DateUtils {
  ...
  static chatTimeShow(
    int ms,) {
   // ms = ms - 3* 24*60*60*1000;
    if (isToday(ms)) {
      return formatDateMs(ms, format: DateFormats.h_m);
    } else if (isYesterdayByMs(ms, DateUtils.getNowDateMs())) {
      return "昨天 " + formatDateMs(ms, format: DateFormats.h_m);
    } else if (isWeek(ms)) {
      return getWeekdayByMs(ms,languageCode: 'zh') +
          " " +
          formatDateMs(ms, format: DateFormats.h_m,);
    } else {
      return formatDateMs(ms, format: DateFormats.y_mo_d_h_m);
    }
  }
